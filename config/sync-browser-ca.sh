...
NSS_DB="${2:-/home/tau/.pki/nssdb}"
NICKNAME="tau-sandbox microsandbox CA"
...
(
    ...
    if certutil -L -d "sql:$NSS_DB" -n "$NICKNAME" >/dev/null 2>&1; then
        ...
        installed_trust="$(certutil -L -d "sql:$NSS_DB" | sed -n 's/^tau-sandbox microsandbox CA  *\([^ ]*\)  *$/\1/p')"
    fi
