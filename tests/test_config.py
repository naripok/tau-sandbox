...
def test_append_system_doc_lists_installed_tools():
    text = _read("APPEND_SYSTEM.md")
    for tool in ("Python", "uv", "Node.js", "opencode", "git", "ast-grep", "ripgrep", "certutil", "browser", "Chromium"):
        assert tool in text
...
def _nss_certificate_sha(database: pathlib.Path) -> str:
    exported = subprocess.run(
        [
            ...
            "-n", "opencode-sandbox microsandbox CA", "-r",
        ],
        check=True,
        capture_output=True,
    ).stdout
...
def test_entrypoint_syncs_rotates_and_removes_microsandbox_ca(tmp_path):
    ...
    subprocess.run(
        [
            ...
            "-n", "opencode-sandbox microsandbox CA", "-t", ",,",
        ],
        check=True,
    )
    _run_browser_ca_sync(tmp_path, first)
    ...
    assert "opencode-sandbox microsandbox CA" in listing
    assert "C,," in listing
    ...
    staging = subprocess.run(
        [
            "certutil", "-L", "-d", f"sql:{database}",
            "-n", "opencode-sandbox microsandbox CA (staging)",
        ],
        capture_output=True,
    )
    ...
    removed = subprocess.run(
        [
            "certutil", "-L", "-d", f"sql:{database}",
            "-n", "opencode-sandbox microsandbox CA",
        ],
        capture_output=True,
    )
...
def test_entrypoint_serializes_browser_ca_updates(tmp_path):
    ...
    assert listing.count("opencode-sandbox microsandbox CA ") == 1
...
def test_entrypoint_runs_browser_ca_sync_helper_with_fixed_paths():
    ...
    assert "/home/opencode/.pki/nssdb" in text
